"""
generate_model_adjustment_release_resolution_wave_packet_review_session_brief.py

v5.8-slice-1 — Read-only controlled release resolution-wave-packet-review-session-brief
generator.

Consumes the frozen v5.7 release resolution wave packet review session pack output
and emits one deterministic brief entry per session-pack record.
...
SOURCE_PATHS = {
    "model_adjustment_release_resolution_wave_packet_review_session_pack_json": (
        "ops/model_adjustments/model_adjustment_release_resolution_wave_packet_review_session_pack.json"
    )
}

OUTPUT_JSON = os.path.join(
    OPS_DIR,
    "model_adjustment_release_resolution_wave_packet_review_session_brief.json",
)
OUTPUT_MD = os.path.join(
    OPS_DIR,
    "model_adjustment_release_resolution_wave_packet_review_session_brief.md",
)

VERSION = "v5.8-slice-1"
EXPECTED_REVIEW_SESSION_PACK_COUNT = 3
...
def normalize_non_empty_string(value, field_name: str, index: int) -> str:
    if not isinstance(value, str):
        raise ValueError(
            f"session-pack record[{index}] field {field_name!r} must be str; "
            "fail-closed — manual review required"
        )
    normalized = value.strip()
    if not normalized:
        raise ValueError(
            f"session-pack record[{index}] field {field_name!r} must be non-empty; "
            "fail-closed — manual review required"
        )
    return normalized
...
def normalize_non_negative_int(value, field_name: str, index: int) -> int:
    if isinstance(value, bool):
        raise ValueError(
            f"session-pack record[{index}] field {field_name!r} must be int, not bool; "
            "fail-closed — manual review required"
        )
    try:
        ...
        raise ValueError(
            f"session-pack record[{index}] field {field_name!r} malformed int={value!r}; "
            "fail-closed — manual review required"
        ) from exc
    if normalized < 0:
        raise ValueError(
            f"session-pack record[{index}] field {field_name!r} must be >= 0; "
            "fail-closed — manual review required"
        )
    return normalized
...
def normalize_bool(value, field_name: str, index: int) -> bool:
    if not isinstance(value, bool):
        raise ValueError(
            f"session-pack record[{index}] field {field_name!r} must be bool; "
            "fail-closed — manual review required"
        )
    return value
...
def normalize_string_list(value, field_name: str, index: int) -> list:
    if not isinstance(value, list):
        raise ValueError(
            f"session-pack record[{index}] field {field_name!r} must be list; "
            "fail-closed — manual review required"
        )
    ...
    for item_index, item in enumerate(value):
        if not isinstance(item, str):
            raise ValueError(
                f"session-pack record[{index}] field {field_name!r} has non-str item at "
                f"index {item_index}; fail-closed — manual review required"
            )
        item_value = item.strip()
        if not item_value:
            raise ValueError(
                f"session-pack record[{index}] field {field_name!r} has empty item at "
                f"index {item_index}; fail-closed — manual review required"
            )
...
def validate_upstream_payload(source_data: dict) -> None:
    if not isinstance(source_data, dict):
        raise ValueError("upstream session pack payload is not a dict; fail-closed")

    version = source_data.get(
        "model_adjustment_release_resolution_wave_packet_review_session_pack_version"
    )
    if not isinstance(version, str) or not version.strip():
        raise ValueError(
            "upstream session pack version missing or malformed; fail-closed"
        )

    session_pack_records = source_data.get("release_resolution_wave_packet_review_session_pack")
    if not isinstance(session_pack_records, list):
        raise ValueError(
            "upstream release_resolution_wave_packet_review_session_pack missing or malformed; "
            "fail-closed"
        )


def validate_session_pack_record(record: dict, index: int) -> None:
    if not isinstance(record, dict):
        raise ValueError(
            f"session-pack record[{index}] malformed type={type(record).__name__}; "
            "fail-closed — manual review required"
        )

    for field in REQUIRED_SESSION_PACK_FIELDS:
        if field not in record:
            raise ValueError(
                f"session-pack record[{index}] missing required field {field!r}; "
                "fail-closed — manual review required"
            )
    ...
    if wave_type not in ("prohibition_cluster", "blocker_cluster", "remaining_resolution_cluster"):
        raise ValueError(
            f"session-pack record[{index}] has unknown wave_type={wave_type!r}; "
            "fail-closed — manual review required"
        )
    ...
    if review_lane not in REVIEW_LANE_ORDER:
        raise ValueError(
            f"session-pack record[{index}] has unknown review_lane={review_lane!r}; "
            "fail-closed — manual review required"
        )


def sorted_session_pack_records(records: list) -> list:
    return sorted(
        records,
        key=lambda record: (
            REVIEW_LANE_ORDER[record["review_lane"]],
            record["review_session_pack_priority"],
            record["wave_rank"],
            record["packet_priority"],
            record["checklist_priority"],
            record["resolution_wave_packet_review_session_pack_id"],
            record["source_resolution_wave_packet_review_agenda_id"],
        ),
    )


def build_session_brief_records(records: list) -> list:
    for index, record in enumerate(records):
        validate_session_pack_record(record, index)

    output = []

    for index, record in enumerate(sorted_session_pack_records(records), start=1):
        output.append(
            {
                "resolution_wave_packet_review_session_brief_id": (
                    f"resolution-wave-packet-review-session-brief-{index:04d}"
                ),
                ...
                "review_session_brief_priority": record["review_session_pack_priority"],
                "brief_position": index,
                ...
                "review_session_brief_notes": (
                    "v5_8_read_only_review_session_brief_projection_of_v5_7_session_pack"
...
def validate_session_brief(records: list, session_pack_records: list) -> None:
    sorted_packs = sorted_session_pack_records(session_pack_records)

    if len(records) != len(sorted_packs):
        raise ValueError(
            f"session brief count {len(records)} does not match "
            f"sorted session pack count {len(sorted_packs)}; fail-closed"
        )
    ...
    expected_pack_order = [
        record.get("resolution_wave_packet_review_session_pack_id", "")
        for record in sorted_packs
    ]
    if source_pack_ids != expected_pack_order:
        raise ValueError(
            "session brief records are not in deterministic session-pack sort order; fail-closed"
        )

    brief_ids = [record["resolution_wave_packet_review_session_brief_id"] for record in records]
    if len(brief_ids) != len(set(brief_ids)):
        raise ValueError(
            "duplicate resolution_wave_packet_review_session_brief_id in output; fail-closed"
        )

    if len(source_pack_ids) != len(set(source_pack_ids)):
        raise ValueError(
            "duplicate source_resolution_wave_packet_review_session_pack_id in session brief; fail-closed"
        )

    upstream_pack_ids = [
        record.get("resolution_wave_packet_review_session_pack_id", "")
        for record in session_pack_records
    ]
    if len(upstream_pack_ids) != EXPECTED_REVIEW_SESSION_PACK_COUNT:
        raise ValueError(
            f"upstream session pack count {len(upstream_pack_ids)} does not match "
            f"EXPECTED_REVIEW_SESSION_PACK_COUNT={EXPECTED_REVIEW_SESSION_PACK_COUNT}; fail-closed"
        )

    if set(source_pack_ids) != set(upstream_pack_ids):
        raise ValueError(
            "session brief coverage does not match upstream session pack records; "
            "fail-closed"
        )

    for record in records:
        if record.get("review_session_brief_priority") != record.get(
            "review_session_pack_priority"
        ):
            raise ValueError(
                "review_session_brief_priority drift for "
                f"{record['resolution_wave_packet_review_session_brief_id']}; fail-closed"
            )
...
def build_summary(records: list, session_pack_records: list) -> dict:
    lane_counts = {
    ...
    return {
        "upstream_session_pack_count": len(session_pack_records),
        "total_review_session_brief_records": len(records),
...
def build_source_versions(source_data: dict) -> dict:
    return {
        "model_adjustment_release_resolution_wave_packet_review_session_pack_version": source_data.get(
            "model_adjustment_release_resolution_wave_packet_review_session_pack_version", ""
        )
    }
...
def build_markdown_lines(
    ...
    lines = [
        "# Model Adjustment Release Resolution Wave Packet Review Session Brief",
        "",
        f"**Version**: {VERSION}",
    ...
    lines.append("")
    for record in records:
        lines.append(f"## {record['resolution_wave_packet_review_session_brief_id']}")
        lines.append("")
        lines.append(
            "- source_resolution_wave_packet_review_session_pack_id: "
            f"{record['source_resolution_wave_packet_review_session_pack_id']}"
        )
        lines.append(
        ...
        lines.append(
            f"- review_session_pack_priority: {record['review_session_pack_priority']}"
        )
        lines.append(f"- review_lane: {record['review_lane']}")
        lines.append(
            f"- review_session_brief_priority: {record['review_session_brief_priority']}"
        )
        lines.append(f"- brief_position: {record['brief_position']}")
...
def main():
    ...
    session_pack_records = source_data.get(
        "release_resolution_wave_packet_review_session_pack", []
    )
    records = build_session_brief_records(session_pack_records)
    validate_session_brief(records, session_pack_records)
    summary = build_summary(records, session_pack_records)
    ...
    payload = {
        ...
        "model_adjustment_release_resolution_wave_packet_review_session_brief_version": VERSION,
        "release_resolution_wave_packet_review_session_brief_summary": summary,
        "release_resolution_wave_packet_review_session_brief": records,
    ...
    print(
        "[STATUS] "
        f"review_session_brief_records={summary['total_review_session_brief_records']} "
        f"upstream_session_pack_count={summary['upstream_session_pack_count']} "
