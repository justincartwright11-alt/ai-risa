"""
generate_model_adjustment_release_resolution_wave_packet_review_docket.py

v5.5-slice-1 — Read-only controlled release resolution-wave-packet-review-docket
generator.

Consumes the frozen v5.4 release resolution wave packet review board output and
emits one deterministic review-docket entry per review-board record.
...
SOURCE_PATHS = {
    "model_adjustment_release_resolution_wave_packet_review_board_json": (
        "ops/model_adjustments/model_adjustment_release_resolution_wave_packet_review_board.json"
    )
}

OUTPUT_JSON = os.path.join(
    OPS_DIR,
    "model_adjustment_release_resolution_wave_packet_review_docket.json",
)
OUTPUT_MD = os.path.join(
    OPS_DIR,
    "model_adjustment_release_resolution_wave_packet_review_docket.md",
)

VERSION = "v5.5-slice-1"
EXPECTED_REVIEW_BOARD_COUNT = 3
...
def normalize_non_empty_string(value, field_name: str, index: int) -> str:
    if not isinstance(value, str):
        raise ValueError(
            f"review-board record[{index}] field {field_name!r} must be str; "
            "fail-closed — manual review required"
        )
    normalized = value.strip()
    if not normalized:
        raise ValueError(
            f"review-board record[{index}] field {field_name!r} must be non-empty; "
            "fail-closed — manual review required"
        )
    return normalized
...
def normalize_non_negative_int(value, field_name: str, index: int) -> int:
    if isinstance(value, bool):
        raise ValueError(
            f"review-board record[{index}] field {field_name!r} must be int, not bool; "
            "fail-closed — manual review required"
        )
    try:
        ...
        raise ValueError(
            f"review-board record[{index}] field {field_name!r} malformed int={value!r}; "
            "fail-closed — manual review required"
        ) from exc
    if normalized < 0:
        raise ValueError(
            f"review-board record[{index}] field {field_name!r} must be >= 0; "
            "fail-closed — manual review required"
        )
    return normalized
...
def normalize_bool(value, field_name: str, index: int) -> bool:
    if not isinstance(value, bool):
        raise ValueError(
            f"review-board record[{index}] field {field_name!r} must be bool; "
            "fail-closed — manual review required"
        )
    return value
...
def normalize_string_list(value, field_name: str, index: int) -> list:
    if not isinstance(value, list):
        raise ValueError(
            f"review-board record[{index}] field {field_name!r} must be list; "
            "fail-closed — manual review required"
        )
    ...
    for item_index, item in enumerate(value):
        if not isinstance(item, str):
            raise ValueError(
                f"review-board record[{index}] field {field_name!r} has non-str item at "
                f"index {item_index}; fail-closed — manual review required"
            )
        item_value = item.strip()
        if not item_value:
            raise ValueError(
                f"review-board record[{index}] field {field_name!r} has empty item at "
                f"index {item_index}; fail-closed — manual review required"
            )
...
def validate_upstream_payload(source_data: dict) -> None:
    if not isinstance(source_data, dict):
        raise ValueError("upstream review-board payload is not a dict; fail-closed")

    version = source_data.get(
        "model_adjustment_release_resolution_wave_packet_review_board_version"
    )
    if not isinstance(version, str) or not version.strip():
        raise ValueError(
            "upstream review-board version missing or malformed; fail-closed"
        )

    board_records = source_data.get("release_resolution_wave_packet_review_board")
    if not isinstance(board_records, list):
        raise ValueError(
            "upstream release_resolution_wave_packet_review_board missing or malformed; "
            "fail-closed"
        )


def validate_review_board_record(record: dict, index: int) -> None:
    if not isinstance(record, dict):
        raise ValueError(
            f"review-board record[{index}] malformed type={type(record).__name__}; "
            "fail-closed — manual review required"
        )

    for field in REQUIRED_REVIEW_BOARD_FIELDS:
        if field not in record:
            raise ValueError(
                f"review-board record[{index}] missing required field {field!r}; "
                "fail-closed — manual review required"
            )
    ...
    if wave_type not in WAVE_TYPE_ORDER:
        raise ValueError(
            f"review-board record[{index}] has unknown wave_type={wave_type!r}; "
            "fail-closed — manual review required"
        )
    ...
    if review_lane not in REVIEW_LANE_ORDER:
        raise ValueError(
            f"review-board record[{index}] has unknown review_lane={review_lane!r}; "
            "fail-closed — manual review required"
        )
...
def build_review_docket_records(records: list) -> list:
    for index, record in enumerate(records):
        validate_review_board_record(record, index)

    output = []

    for index, record in enumerate(sorted_review_board_records(records), start=1):
        output.append(
            {
                "resolution_wave_packet_review_docket_id": (
                    f"resolution-wave-packet-review-docket-{index:04d}"
                ),
                ...
                "review_docket_priority": record["review_board_priority"],
                "docket_position": index,
                ...
                "review_docket_notes": (
                    "v5_5_read_only_review_docket_projection_of_v5_4_review_board"
...
def validate_review_docket(records: list, review_board_records: list) -> None:
    ...
    source_board_ids = [
        record["source_resolution_wave_packet_review_board_id"] for record in records
    ]
    if len(source_board_ids) != len(set(source_board_ids)):
        raise ValueError(
            "duplicate source_resolution_wave_packet_review_board_id in docket; fail-closed"
        )

    upstream_board_ids = [
        record.get("resolution_wave_packet_review_board_id", "")
        for record in review_board_records
    ]
    if len(upstream_board_ids) != EXPECTED_REVIEW_BOARD_COUNT:
        raise ValueError(
            f"upstream review-board count {len(upstream_board_ids)} does not match "
            f"EXPECTED_REVIEW_BOARD_COUNT={EXPECTED_REVIEW_BOARD_COUNT}; fail-closed"
        )

    if set(source_board_ids) != set(upstream_board_ids):
        raise ValueError(
            "review docket coverage does not match upstream review-board records; "
            "fail-closed"
        )
        ...
        for field in (
            ...
            if len(values) != len(set(values)):
                raise ValueError(
                    ...
                    f"{record['resolution_wave_packet_review_docket_id']}; fail-closed"
                )
...
def build_summary(records: list, review_board_records: list) -> dict:
    lane_counts = {
    ...
    return {
        "upstream_review_board_count": len(review_board_records),
        "total_review_docket_records": len(records),
...
def build_source_versions(source_data: dict) -> dict:
    return {
        "model_adjustment_release_resolution_wave_packet_review_board_version": source_data.get(
            "model_adjustment_release_resolution_wave_packet_review_board_version", ""
        )
    }
...
def build_markdown_lines(
    ...
    lines = [
        "# Model Adjustment Release Resolution Wave Packet Review Docket",
        "",
        f"**Version**: {VERSION}",
    ...
    lines.append("")
    for record in records:
        lines.append(f"## {record['resolution_wave_packet_review_docket_id']}")
        lines.append("")
        lines.append(
        ...
        lines.append(f"- docket_position: {record['docket_position']}")
...
def main():
    ...
    source_data, errors["model_adjustment_release_resolution_wave_packet_review_board_json"] = load_source(
        "model_adjustment_release_resolution_wave_packet_review_board_json"
    )
    ...
    review_board_records = source_data.get("release_resolution_wave_packet_review_board", [])
    records = build_review_docket_records(review_board_records)
    validate_review_docket(records, review_board_records)
    summary = build_summary(records, review_board_records)
    ...
    payload = {
        ...
        "model_adjustment_release_resolution_wave_packet_review_docket_version": VERSION,
        "release_resolution_wave_packet_review_docket_summary": summary,
        "release_resolution_wave_packet_review_docket": records,
    ...
    print(
        "[STATUS] "
        f"review_docket_records={summary['total_review_docket_records']} "
        f"upstream_review_board_count={summary['upstream_review_board_count']} "
