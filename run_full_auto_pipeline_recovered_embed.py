# This file is a direct copy of the restored charts script, ready for exporter embedding work.
...
# --- Visual-data metric helpers ---
def build_radar_metrics(fight_report):
	# Compact comparative values (example logic, can be refined per bout)
	return {
		"structure_control": "Elite",
		"energy_efficiency": "High",
		"fatigue_resilience": "Strong",
		"mental_stability": "Composed",
		"disruption_risk": "Low",
		"scoring_clarity": "Clear"
	}

def build_round_flow_metrics(fight_report):
	return {
		"early_control": "Price establishes range, clean exchanges",
		"mid_fight_separation": "Aquino surges, swing rounds possible",
		"late_fight_stability": "Price manages risk, Aquino forces chaos"
	}

def build_heatmap_metrics(fight_report):
	return {
		"clinch_disruption": "Medium",
		"tempo_spike_risk": "Elevated",
		"range_breakdown": "Low",
		"late_pressure_window": "Aquino late surge risk"
	}

# --- Helper: Parse markdown image line ---
def parse_markdown_image_line(line):
	"""
	Parse a markdown image line of the form:
	  ![Alt text](path/to/image.png)
	Returns (alt_text, path) if valid, else None.
	Handles local paths, forward/back slashes, and whitespace.
	"""
	import re
	...
	if mode == "event" and report.replace('_', ' ') in report_names:
		...
		for bout in bout_entries:
			...
			if fight_report:
				...
				# --- Visual-data metric blocks (only for resolved premium bouts) ---
				if fight_report.get('bout_title', '').strip().lower() == 'lauren price vs. stephanie pineiro aquino':
					# Radar Metrics
					radar = build_radar_metrics(fight_report)
					section_lines.append('### Radar Metrics')
					section_lines.append('| Metric | Value |')
					section_lines.append('|---|---|')
					for k, v in radar.items():
						section_lines.append(f'| {k.replace("_", " ").title()} | {v} |')
					section_lines.append('')
					# Round-Flow Metrics
					roundflow = build_round_flow_metrics(fight_report)
					section_lines.append('### Round-Flow Metrics')
					section_lines.append('| Phase | Value |')
					section_lines.append('|---|---|')
					for k, v in roundflow.items():
						section_lines.append(f'| {k.replace("_", " ").title()} | {v} |')
					section_lines.append('')
					# Danger-Zone Heat Map Metrics
					heatmap = build_heatmap_metrics(fight_report)
					section_lines.append('### Danger-Zone Heat Map Metrics')
					section_lines.append('| Zone | Risk |')
					section_lines.append('|---|---|')
		...
		for para in full_report_md.split("\n\n"):
			# DOCX image embedding: handle markdown image lines
			lines = para.split("\n")
			for line in lines:
				img_info = parse_markdown_image_line(line)
				if img_info:
					alt, img_path = img_info
					img_path = img_path.replace("\\", "/").strip()
					candidates = [img_path]
					if not os.path.isabs(img_path):
						reports_dir = os.path.dirname(md_path)
						candidates.append(os.path.join(reports_dir, img_path))
					img_file = next((p for p in candidates if os.path.exists(p)), None)
					if img_file:
						try:
							doc.add_picture(img_file, width=Inches(4.5))
						except Exception:
							pass
					# Always skip the raw image line
					continue
				# Not an image line: keep existing logic
		...
		if fmt == "pdf":
			...
			for para in full_report_md.split("\n\n"):
				lines = para.split("\n")
				for line in lines:
					# Normalize tabs, strip trailing whitespace
					...
					img_info = parse_markdown_image_line(line)
					if img_info:
						# Image embedding already handled above
						continue
					# Defensive: skip lines that are too long and unbreakable
					max_width = pdf.w - pdf.l_margin - pdf.r_margin
					if len(line) > 200 and ' ' not in line:
						# Truncate or split
						for i in range(0, len(line), 80):
							chunk = line[i:i+80]
							if pdf.get_x() + pdf.get_string_width(chunk) > max_width:
								pdf.ln(5)
							pdf.multi_cell(0, 10, chunk)
						continue
					# Defensive: check available width before writing
					try:
						if pdf.get_x() + pdf.get_string_width(line) > max_width:
							pdf.ln(5)
						pdf.multi_cell(0, 10, line)
					except Exception:
						# If still not renderable, skip silently
						continue
			pdf.output(f"C:/ai_risa_data/reports/{args.event}_full_prim_report.pdf")
	else:
