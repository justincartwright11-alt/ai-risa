# PDF patch with fallback forced-splitting for unbreakable lines
# Only applies in the PDF exporter
...
from run_full_auto_pipeline_recovered_embed import resolve_bout_identity, is_true_tba_bout, build_fight_report, parse_markdown_image_line

def debug_print(msg, *args):
    print(f"[PDFPATCH3] {msg}", *args)

def sanitize_markdown_for_pdf(md):
    sanitized = []
    for line in md.split('\n'):
        # Remove raw image lines
        if line.strip().startswith('!['):
            continue
        # Convert long table rows to plain text
        if line.strip().startswith('|') and len(line) > 120:
            cells = [c.strip() for c in line.strip('|').split('|')]
            sanitized.append(' | '.join(cells))
            continue
        # Break up any no-space string > 100 chars
        tokens = line.split(' ')
        new_line = []
        for token in tokens:
            if len(token) > 100:
                for i in range(0, len(token), 80):
                    new_line.append(token[i:i+80])
            else:
                new_line.append(token)
        sanitized.append(' '.join(new_line))
    return '\n'.join(sanitized)

def safe_pdf_chunks(line, max_chunk=80):
    line = (line or "").replace("\t", " ").rstrip()
    if not line:
        return [""]
    if " " in line or len(line) <= max_chunk:
        return [line]
    # Debug log for forced split
    print(f"[PDFPATCH3] Forced split: {line[:80]}... (len={len(line)})", file=sys.stderr)
    return [line[i:i+max_chunk] for i in range(0, len(line), max_chunk)]
    ...
    if mode == 'event' and report.replace('_', ' ') in report_names:
        ...
        for i, bout in enumerate(bout_entries):
            ...
            if fight_report:
                ...
                from run_full_auto_pipeline_recovered_embed import render_full_prim_fight_section
                premium_md_blocks.append(render_full_prim_fight_section(fight_report))
        ...
        if premium_sections or tba_sections:
            try:
                ...
                debug_print('Markdown write failed', str(e))
            try:
                ...
                debug_print('DOCX write failed', str(e))
            if fmt == 'pdf':
                try:
                    ...
                    sanitized_md = sanitize_markdown_for_pdf(full_report_md)
                    for para in sanitized_md.split("\n\n"):
                        for line in para.split("\n"):
                            parsed = parse_markdown_image_line(line)
                            if parsed:
                                continue
                            ...
                            if not clean.strip():
                                pdf.ln(4)
                                continue
                            for chunk in safe_pdf_chunks(clean, 80):
                                if not chunk:
                                    pdf.ln(4)
                                    continue
                                try:
                    ...
                    debug_print('PDF write failed', str(e))
        else:
