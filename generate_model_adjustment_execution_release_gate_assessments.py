#!/usr/bin/env python3
"""
AI-RISA v3.7 Controlled Execution Release Gate Assessments (slice 1): read-only governance layer.

Builds a canonical release-gate assessment register that determines whether release
is open, closed, blocked, or structurally unavailable, and what requirements/evidence
remain unmet. This script is governance-only and does not execute adjustments,
auto-promote state, write configs, or mutate model behavior.
"""
...
OUTPUT_JSON = "model_adjustment_execution_release_gate_assessments.json"
OUTPUT_MD = "model_adjustment_execution_release_gate_assessments.md"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate read-only controlled execution release-gate assessment artifacts"
    )
...
def derive_gate_state(
    release_authority_state: str,
    authority_availability: str,
    unmet_gate_requirements: list[str],
) -> tuple[str, str]:
    if authority_availability in {"absent", "expired", "structurally_unavailable"}:
        return "structurally_unavailable", "gate_blocked_by_authority_unavailability"
    if release_authority_state != "authority_granted":
        return "blocked", "gate_blocked_pending_authority_grant"
    if unmet_gate_requirements:
        return "closed", "gate_closed_pending_unmet_requirements"
    return "open", "gate_open_ready_for_release_review"


def build_release_gate_assessments(
    ...
    seq = 0
    for authority_row in release_authority_rows:
        if not isinstance(authority_row, dict):
            continue

        proposal_id = str(authority_row.get("proposal_id") or "").strip()
        if not proposal_id:
            continue
        ...
        policy_id = authority_row.get("policy_id")
        criteria_id = authority_row.get("criteria_id")
        ineligibility_id = authority_row.get("ineligibility_id")
        prohibition_id = authority_row.get("prohibition_id")
        release_condition_id = authority_row.get("release_condition_id")
        release_authority_id = authority_row.get("release_authority_id")
        ...
        unmet_gate_requirements = list(
            release_condition_row.get("currently_unmet_release_conditions") or []
        )
        required_gate_evidence = list(
            release_condition_row.get("required_release_evidence") or []
        )

        # Add authority-driven unmet requirement when authority is not granted.
        if str(authority_row.get("release_authority_state") or "") != "authority_granted":
            unmet_gate_requirements.append("release_authority_must_be_granted")

        required_release_authority = str(
            authority_row.get("required_release_authority")
            or release_condition_row.get("required_release_authority")
            or "governance_release_board"
        )
        required_operator_role = str(
            authority_row.get("required_operator_role")
            or release_condition_row.get("required_operator_role")
            or "ops_reviewer"
        )

        authority_availability = str(
            authority_row.get("authority_availability") or "structurally_unavailable"
        )
        authority_state = str(
            authority_row.get("release_authority_state") or "authority_not_granted"
        )

        release_gate_state, gate_assessment_status = derive_gate_state(
            release_authority_state=authority_state,
            authority_availability=authority_availability,
            unmet_gate_requirements=unmet_gate_requirements,
        )

        gate_blockers = set(str(x) for x in (authority_row.get("authority_blockers") or []))
        gate_blockers.update(
            str(x)
            for x in (release_condition_row.get("release_blockers") or [])
            if str(x).strip()
        )
        gate_blockers.add(f"release_gate_state:{release_gate_state}")
        gate_blockers.add(f"authority_availability:{authority_availability}")

        gate_state_counts[release_gate_state] = gate_state_counts.get(release_gate_state, 0) + 1
        gate_status_counts[gate_assessment_status] = gate_status_counts.get(gate_assessment_status, 0) + 1
        ...
        record_source_paths: dict[str, Any] = {
            ...
            "execution_ineligibility_id": inelig_row.get("ineligibility_id"),
        ...
        records.append(
            {
                "release_gate_id": f"gate-{seq:04d}",
                ...
                "gate_assessment_status": gate_assessment_status,
                "unmet_gate_requirements": sorted(set(unmet_gate_requirements)),
                "required_gate_evidence": sorted(set(required_gate_evidence)),
                ...
                "gate_notes": "governance_release_gate_assessment_only_no_execution_in_v3_7",
    ...
    expected_records = len(release_authority_rows) if release_authority_rows else len(proposal_ids_in_source)

    summary = {
        "total_proposals_in_source": len(proposal_ids_in_source),
        "total_release_gate_records": len(records),
        ...
        "release_gate_state_counts": gate_state_counts,
        "gate_assessment_status_counts": gate_status_counts,
    }

    return {
        "generated_at_utc": now_utc_iso(),
        "model_adjustment_execution_release_gate_assessments_version": "v3.7-slice-1",
        "source_versions": {
        ...
        "execution_release_gate_assessments_summary": summary,
        "execution_release_gate_assessments": records,
        "source_paths": source_paths,
    }


def render_markdown(payload: dict[str, Any]) -> str:
    summary = payload.get("execution_release_gate_assessments_summary") or {}
    source_versions = payload.get("source_versions") or {}
    rows = payload.get("execution_release_gate_assessments") or []

    lines: list[str] = []
    lines.append("# AI-RISA Controlled Execution Release Gate Assessments (Slice 1)")
    lines.append("")
    lines.append(f"Generated (UTC): {payload.get('generated_at_utc')}")
    lines.append(
        "Release Gate Assessments Version: "
        f"{payload.get('model_adjustment_execution_release_gate_assessments_version')}"
    )
    lines.append(f"Source Versions: {source_versions}")
    lines.append("")
    lines.append("## Release Gate Assessment Summary")
    lines.append(f"- Source Proposals: {summary.get('total_proposals_in_source')}")
    lines.append(f"- Total Release Gate Records: {summary.get('total_release_gate_records')}")
    ...
    lines.append(f"- Release Gate State Counts: {summary.get('release_gate_state_counts')}")
    lines.append(f"- Gate Assessment Status Counts: {summary.get('gate_assessment_status_counts')}")
    lines.append("")
    lines.append("## Execution Release Gate Assessment Records")

    if rows:
        for row in rows:
            lines.append(
                f"- {row.get('release_gate_id')}: {row.get('proposal_id')} | "
            ...
            lines.append(
                f"  Gate State: {row.get('release_gate_state')} | "
                f"Status: {row.get('gate_assessment_status')}"
            )
            lines.append(f"  Unmet Gate Requirements: {row.get('unmet_gate_requirements')}")
            lines.append(f"  Required Gate Evidence: {row.get('required_gate_evidence')}")
            ...
            lines.append(f"  Gate Blockers: {row.get('gate_blockers')}")
            lines.append(f"  Rollback Reference: {row.get('rollback_reference')}")
            lines.append(f"  Gate Notes: {row.get('gate_notes')}")
...
def main() -> int:
    ...
    payload = build_release_gate_assessments(
    ...
    s = payload.get("execution_release_gate_assessments_summary") or {}
    print(
        "[STATUS] release_gates={total} proposals_covered={covered} all_covered={all_covered} "
        "record_count_matches_expected={match} release_gate_state={state}".format(
            total=s.get("total_release_gate_records"),
            covered=s.get("proposals_covered"),
            all_covered=s.get("all_proposals_covered"),
            match=s.get("record_count_matches_expected"),
            state=s.get("release_gate_state_counts"),
        )
    )
    return 0
