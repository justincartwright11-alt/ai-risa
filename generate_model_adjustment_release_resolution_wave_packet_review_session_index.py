...
def get_required_field(record, field_name, record_index):
    if field_name not in record:
        raise ValueError(f"Missing required field '{field_name}' in catalog record {record_index}")
...
def validate_upstream_payload(payload, path):
    ...
    version = payload.get("model_adjustment_release_resolution_wave_packet_review_session_catalog_version")
    if version != "v6.6-slice-1":
        raise ValueError(f"Expected catalog version 'v6.6-slice-1', got {repr(version)}")

    if "release_resolution_wave_packet_review_session_catalog" not in payload:
        raise ValueError("Missing upstream field: release_resolution_wave_packet_review_session_catalog")

    records = payload["release_resolution_wave_packet_review_session_catalog"]
    if not isinstance(records, list):
        raise ValueError(
            "release_resolution_wave_packet_review_session_catalog is not a list: "
            f"{type(records).__name__}"
        )
    if len(records) == 0:
        raise ValueError("No catalog records found in upstream payload")
    return records


def build_index_records(catalog_records):
    index_records = []

    for idx, catalog_record in enumerate(catalog_records):
        if not isinstance(catalog_record, dict):
            raise ValueError(
                f"Validation error in catalog record {idx}: expected object, got {type(catalog_record).__name__}"
            )

        try:
            catalog_id = normalize_string(
                get_required_field(catalog_record, "resolution_wave_packet_review_session_catalog_id", idx),
                "catalog_id",
            )
            archive_id = normalize_string(
                get_required_field(catalog_record, "source_resolution_wave_packet_review_session_archive_id", idx),
                "archive_id",
            )
            journal_id = normalize_string(
                get_required_field(catalog_record, "source_resolution_wave_packet_review_session_journal_id", idx),
                "journal_id",
            )
            ledger_id = normalize_string(
                get_required_field(catalog_record, "source_resolution_wave_packet_review_session_ledger_id", idx),
                "ledger_id",
            )
            register_id = normalize_string(
                get_required_field(catalog_record, "source_resolution_wave_packet_review_session_register_id", idx),
                "register_id",
            )
            receipt_id = normalize_string(
                get_required_field(catalog_record, "source_resolution_wave_packet_review_session_receipt_id", idx),
                "receipt_id",
            )
            intake_id = normalize_string(
                get_required_field(catalog_record, "source_resolution_wave_packet_review_session_intake_id", idx),
                "intake_id",
            )
            handoff_id = normalize_string(
                get_required_field(catalog_record, "source_resolution_wave_packet_review_session_handoff_id", idx),
                "handoff_id",
            )
            brief_id = normalize_string(
                get_required_field(catalog_record, "source_resolution_wave_packet_review_session_brief_id", idx),
                "brief_id",
            )
            pack_id = normalize_string(
                get_required_field(catalog_record, "source_resolution_wave_packet_review_session_pack_id", idx),
                "pack_id",
            )
            agenda_id = normalize_string(
                get_required_field(catalog_record, "source_resolution_wave_packet_review_agenda_id", idx),
                "agenda_id",
            )
            docket_id = normalize_string(
                get_required_field(catalog_record, "source_resolution_wave_packet_review_docket_id", idx),
                "docket_id",
            )
            board_id = normalize_string(
                get_required_field(catalog_record, "source_resolution_wave_packet_review_board_id", idx),
                "board_id",
            )
            checklist_id = normalize_string(
                get_required_field(catalog_record, "source_resolution_wave_packet_checklist_id", idx),
                "checklist_id",
            )
            packet_id = normalize_string(
                get_required_field(catalog_record, "source_resolution_wave_packet_id", idx),
                "packet_id",
            )
            wave_id = normalize_string(
                get_required_field(catalog_record, "source_resolution_wave_id", idx),
                "wave_id",
            )

            wave_rank = normalize_int(get_required_field(catalog_record, "wave_rank", idx), "wave_rank")
            wave_type = normalize_string(get_required_field(catalog_record, "wave_type", idx), "wave_type")
            packet_priority = normalize_string(
                get_required_field(catalog_record, "packet_priority", idx),
                "packet_priority",
            )
            checklist_priority = normalize_string(
                get_required_field(catalog_record, "checklist_priority", idx),
                "checklist_priority",
            )
            review_board_priority = normalize_string(
                get_required_field(catalog_record, "review_board_priority", idx),
                "review_board_priority",
            )
            review_docket_priority = normalize_string(
                get_required_field(catalog_record, "review_docket_priority", idx),
                "review_docket_priority",
            )
            review_agenda_priority = normalize_string(
                get_required_field(catalog_record, "review_agenda_priority", idx),
                "review_agenda_priority",
            )
            review_session_pack_priority = normalize_string(
                get_required_field(catalog_record, "review_session_pack_priority", idx),
                "review_session_pack_priority",
            )
            review_session_brief_priority = normalize_string(
                get_required_field(catalog_record, "review_session_brief_priority", idx),
                "review_session_brief_priority",
            )
            review_session_handoff_priority = normalize_string(
                get_required_field(catalog_record, "review_session_handoff_priority", idx),
                "review_session_handoff_priority",
            )
            review_session_intake_priority = normalize_string(
                get_required_field(catalog_record, "review_session_intake_priority", idx),
                "review_session_intake_priority",
            )
            review_session_receipt_priority = normalize_string(
                get_required_field(catalog_record, "review_session_receipt_priority", idx),
                "review_session_receipt_priority",
            )
            review_session_register_priority = normalize_string(
                get_required_field(catalog_record, "review_session_register_priority", idx),
                "review_session_register_priority",
            )
            review_session_ledger_priority = normalize_string(
                get_required_field(catalog_record, "review_session_ledger_priority", idx),
                "review_session_ledger_priority",
            )
            review_session_journal_priority = normalize_string(
                get_required_field(catalog_record, "review_session_journal_priority", idx),
                "review_session_journal_priority",
            )
            review_session_archive_priority = normalize_string(
                get_required_field(catalog_record, "review_session_archive_priority", idx),
                "review_session_archive_priority",
            )
            review_session_catalog_priority = normalize_string(
                get_required_field(catalog_record, "review_session_catalog_priority", idx),
                "review_session_catalog_priority",
            )
            review_lane = normalize_review_lane(
                get_required_field(catalog_record, "review_lane", idx),
                "review_lane",
            )

            member_cluster_ids = normalize_list(
                get_required_field(catalog_record, "member_cluster_ids", idx),
                "member_cluster_ids",
            )
            member_dependency_ids = normalize_list(
                get_required_field(catalog_record, "member_dependency_ids", idx),
                "member_dependency_ids",
            )
            member_source_refs = normalize_list(
                get_required_field(catalog_record, "member_source_refs", idx),
                "member_source_refs",
            )
            affected_proposal_ids = normalize_list(
                get_required_field(catalog_record, "affected_proposal_ids", idx),
                "affected_proposal_ids",
            )
            affected_queue_ids = normalize_list(
                get_required_field(catalog_record, "affected_queue_ids", idx),
                "affected_queue_ids",
            )

            affected_record_count = normalize_int(
                get_required_field(catalog_record, "affected_record_count", idx),
                "affected_record_count",
            )
            cluster_count = normalize_int(
                get_required_field(catalog_record, "cluster_count", idx),
                "cluster_count",
            )
            dependency_count = normalize_int(
                get_required_field(catalog_record, "dependency_count", idx),
                "dependency_count",
            )
            has_prohibition_path = normalize_bool(
                get_required_field(catalog_record, "has_prohibition_path", idx),
                "has_prohibition_path",
            )
            has_blocker_path = normalize_bool(
                get_required_field(catalog_record, "has_blocker_path", idx),
                "has_blocker_path",
            )
            terminal_posture = normalize_string(
                get_required_field(catalog_record, "terminal_posture", idx),
                "terminal_posture",
            )
        except ValueError as e:
            raise ValueError(f"Validation error in catalog record {idx}: {e}")

        index_records.append(
            {
                "resolution_wave_packet_review_session_index_id": (
                    f"resolution-wave-packet-review-session-index-{idx + 1:04d}"
                ),
                ...
                "review_session_index_priority": review_session_catalog_priority,
    ...
    for idx, rec in enumerate(index_records):
        rec["index_position"] = idx + 1
...
| Field | Value |
|---|---|
| upstream_session_catalog_count | {payload['upstream_session_catalog_count']} |
| total_review_session_index_records | {len(records)} |
...
| Source | Version |
|---|---|
| model_adjustment_release_resolution_wave_packet_review_session_catalog_version | {payload['upstream_session_catalog_version']} |
...
def main():
    try:
        ...
        if not os.path.exists(catalog_path):
            raise FileNotFoundError(f"Upstream catalog JSON not found: {catalog_path}")

        with open(catalog_path, "r", encoding="utf-8") as f:
            catalog_payload = json.load(f)

        catalog_records = validate_upstream_payload(catalog_payload, catalog_path)
        index_records = build_index_records(catalog_records)

        coverage_reconciled = len(catalog_records) == len(index_records)
        ...
        output_payload = {
            ...
            "upstream_session_catalog_count": len(catalog_records),
            "upstream_session_catalog_version": catalog_payload.get(
                "model_adjustment_release_resolution_wave_packet_review_session_catalog_version"
            ),
            "release_resolution_wave_packet_review_session_index": index_records,
            "coverage_reconciled": coverage_reconciled,
            "deterministic_ordering": is_sorted,
            "review_lane_counts": lane_counts,
        }

        markdown_output = generate_markdown(output_payload, index_records)
        ...
        print(
            f"[STATUS] review_session_index_records={len(index_records)} "
            f"upstream_session_catalog_count={len(catalog_records)} "
