"""
build_upcoming_schedule_escalation_email_dead_letter_notification_email_manual_intervention_operator_followup_action_resolution_closure_completion_dispatch_reconciler.py

v75.5: Operator follow-up action resolution closure completion dispatch reconciler.
Reads the frozen v75.4 completion dispatch outcome ledger and v75.3 completion dispatch, emits reconciled completion dispatch and reconciliation audit artifacts (JSON/MD).
"""
import json
import hashlib
...
# Outputs
RECONCILED_DISPATCH_JSON = Path("ops/events/upcoming_schedule_manual_intervention_operator_followup_action_resolution_closure_completion_dispatch_reconciled.json")
RECONCILED_DISPATCH_MD = Path("ops/events/upcoming_schedule_manual_intervention_operator_followup_action_resolution_closure_completion_dispatch_reconciled.md")
RECONCILIATION_AUDIT_JSON = Path("ops/events/upcoming_schedule_manual_intervention_operator_followup_action_resolution_closure_completion_dispatch_reconciliation_audit.json")
RECONCILIATION_AUDIT_MD = Path("ops/events/upcoming_schedule_manual_intervention_operator_followup_action_resolution_closure_completion_dispatch_reconciliation_audit.md")
...
def reconcile_dispatch(dispatches: List[Dict[str, Any]], outcomes: List[Dict[str, Any]]):
    # Build lookup for outcome by completion_dispatch_id
    outcome_by_id = {(o["completion_dispatch_id"], o["attempt_number"]): o for o in outcomes}
    reconciled = []
    audit = []
    for d in dispatches:
        key = (d["completion_dispatch_id"], 1)  # Only attempt_number=1 for now
    ...
    reconciled.sort(key=lambda x: (x["completion_dispatch_id"]))
    audit.sort(key=lambda x: (x["completion_dispatch_id"], x["attempt_number"]))
...
def main():
    ...
    write_md(reconciled, RECONCILED_DISPATCH_MD, REQUIRED_DISPATCH_FIELDS, "Operator Follow-Up Action Resolution Closure Completion Dispatch Reconciled", "No reconciled completion dispatch records found.")
    write_md(audit, RECONCILIATION_AUDIT_MD, REQUIRED_AUDIT_FIELDS, "Operator Follow-Up Action Resolution Closure Completion Dispatch Reconciliation Audit", "No reconciliation audit records found.")
