...
<#
.SYNOPSIS
    Launcher for AI-RISA pipeline dry-run execution with timestamped run-history.
    
.DESCRIPTION
    - Creates timestamped run folder (YYYY-MM-DD_HHMMSS)
    - Executes full pipeline in dry-run mode (schedule+batch with --dry-run, others soft-skipped)
    - Stores summaries + log in run folder
    - Updates run index (append-only)
    - Preserves exit codes for Task Scheduler detection
    - Safe for verification without modifying production state
...
.EXAMPLE
    .\schedule_full_pipeline_dry_run.ps1 -RepoRoot 'D:\ai_risa' -Formats 'md pdf'
#>
...
try {
    ...
    $cmdParts = @($python.Exe) + @($python.PrefixArgs) + @('.\run_full_pipeline_auto.py', '--dry-run')
    ...
    $processOutput = & $python.Exe @($python.PrefixArgs) .\run_full_pipeline_auto.py --dry-run --formats $($Formats.Split(' ')) 2>&1
