"""
build_upcoming_schedule_escalation_email_dead_letter_notification_email_manual_intervention_operator_followup_action_resolution_closure_delivery_summary.py

v75.1: Operator follow-up action resolution closure delivery summary.
Reads the frozen closure branch artifacts and emits summary artifacts (JSON/MD).
"""
import json
import hashlib
...
SUMMARY_MD = Path("ops/events/upcoming_schedule_manual_intervention_operator_followup_action_resolution_closure_delivery_summary.md")
...
def build_summary():
    queue = load_json_array(CLOSURE_QUEUE_JSON)
    dispatch = load_json_array(CLOSURE_DISPATCH_JSON)
    ...
    # Outstanding closure items (not delivered, failed, skipped, or blocked)
    outstanding_items = [q for q in queue if q.get("closure_queue_item_id") not in {r.get("closure_queue_item_id") for r in reconciled}]
    summary = {
        ...
        "outstanding_closure_items": len(outstanding_items),
...
def write_md(summary: Dict[str, Any]):
    with open(SUMMARY_MD, "w", encoding="utf-8") as f:
        f.write(f"# Operator Follow-Up Action Resolution Closure Delivery Summary\n\n")
        ...
        for k, v in summary["by_dispatch_state"].items():
            f.write(f"- {k}: {v}\n")
        f.write(f"\n**Outstanding closure items:** {summary['outstanding_closure_items']}\n\n")
        if summary["outstanding_items"]:
            f.write("### Outstanding Items\n")
            for item in summary["outstanding_items"]:
                f.write(f"- {item.get('closure_queue_item_id', '')} ({item.get('dedupe_key', '')})\n")
        else:
            f.write("_No outstanding closure items._\n")
...
def main():
    ...
    write_json(summary)
    write_md(summary)
    print(f"Wrote closure delivery summary.")
