"""
build_upcoming_schedule_escalation_email_dead_letter_notification_email_manual_intervention_operator_followup_action_resolution_closure_completion_finalization_completion_dispatch_reconciler.py

v76.5: Operator follow-up action resolution closure completion finalization completion dispatch reconciler.
Reads the v76.4 completion dispatch outcome ledger and reconciles it into the v76.3 completion dispatch artifact.
...
RECONCILED_DISPATCH_MD = Path("ops/events/upcoming_schedule_manual_intervention_operator_followup_action_resolution_closure_completion_finalization_completion_dispatch_reconciled.md")
...
RECONCILIATION_AUDIT_MD = Path("ops/events/upcoming_schedule_manual_intervention_operator_followup_action_resolution_closure_completion_finalization_completion_dispatch_reconciliation_audit.md")
...
def write_md_dispatch(records: List[Dict[str, Any]], path: Path):
    with open(path, "w", encoding="utf-8") as f:
        f.write(f"# Operator Follow-Up Action Resolution Closure Completion Finalization Completion Dispatch (Reconciled)\n\n")
        if not records:
...
def write_md_audit(audit: List[Dict[str, Any]], path: Path):
    with open(path, "w", encoding="utf-8") as f:
        f.write(f"# Operator Follow-Up Action Resolution Closure Completion Finalization Completion Dispatch Reconciliation Audit\n\n")
        if not audit:
            f.write("_No reconciliation audit records._\n")
            return
        for entry in audit:
            f.write(f"- **Completion Dispatch ID:** {entry['completion_dispatch_id']}\n")
