#!/usr/bin/env python3
"""
AI-RISA v4.2 Controlled Release Ineligibility Register (slice 1): read-only governance layer.

Builds a canonical per-proposal release ineligibility register that consolidates all
active release ineligibility conditions from release decisions, release prohibition,
and release blocker layers.
...
OUTPUT_JSON = "model_adjustment_release_ineligibility_register.json"
OUTPUT_MD = "model_adjustment_release_ineligibility_register.md"
...
RELEASE_BLOCKER_REGISTRY_DEFAULT = Path(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate read-only controlled release ineligibility register artifacts"
    )
    ...
    parser.add_argument(
        "--release-blocker-registry-json",
        default=str(RELEASE_BLOCKER_REGISTRY_DEFAULT),
    )
...
def classify_ineligibility(
    ...
    release_prohibition_state: str,
    release_blocker_category: str,
    release_decision_reason: str,
) -> tuple[str, str]:
    if authority_availability in {"absent", "expired", "structurally_unavailable"}:
        return (
            "release_authority_unavailable",
            "release_authority_not_available_under_current_governance_constraints",
        )
    if release_gate_state in {"blocked", "closed", "structurally_unavailable"}:
        return (
            "release_gate_unmet",
            f"release_gate_state_{release_gate_state}_keeps_release_ineligible",
        )
    if release_decision_state in {"release_denied", "release_not_eligible", "structurally_unreleasable"}:
        return (
            ...
            or f"release_decision_state_{release_decision_state}_keeps_release_ineligible",
        )
    if release_prohibition_state == "prohibited":
        return (
            "release_prohibited",
            "release_prohibition_matrix_reports_prohibited_release_actions",
        )
    if release_blocker_category:
        return (
            "release_blockers_active",
            f"release_blocker_registry_category_{release_blocker_category}_is_active",
        )
    return (
        "release_ineligible",
        "release_ineligibility_conditions_remain_active_under_v4_2_governance",
    )


def build_release_ineligibility_register(
    ...
    release_ineligibility_decisions_payload: dict[str, Any],
    release_prohibition_payload: dict[str, Any],
    release_blocker_payload: dict[str, Any],
    ...
    release_ineligibility_decision_rows = (
        release_ineligibility_decisions_payload.get("release_ineligibility_decisions")
        if isinstance(
            release_ineligibility_decisions_payload.get("release_ineligibility_decisions"), list
        )
        else []
    )
    ...
    release_blocker_rows = (
        release_blocker_payload.get("release_blocker_registry")
        if isinstance(release_blocker_payload.get("release_blocker_registry"), list)
        else []
    )
    ...
    release_inelig_decision_by_pid = index_by_proposal_id(release_ineligibility_decision_rows)
    release_prohibition_by_pid = index_by_proposal_id(release_prohibition_rows)
    release_blocker_by_pid = index_by_proposal_id(release_blocker_rows)

    proposal_ids = sorted(
        {
            str(r.get("proposal_id") or "").strip()
            for r in release_ineligibility_decision_rows
    ...
    for seq, proposal_id in enumerate(proposal_ids, start=1):
        ...
        release_inelig_decision_row = release_inelig_decision_by_pid.get(proposal_id, {})
        release_prohibition_row = release_prohibition_by_pid.get(proposal_id, {})
        release_blocker_row = release_blocker_by_pid.get(proposal_id, {})

        release_ineligibility_state = "release_ineligible"
        release_ineligibility_class, release_ineligibility_reason = classify_ineligibility(
            authority_availability=str(
            ...
            release_prohibition_state=str(
                release_prohibition_row.get("release_prohibition_state") or "prohibited"
            ),
            release_blocker_category=str(release_blocker_row.get("release_blocker_category") or ""),
            release_decision_reason=str(release_decision_row.get("release_decision_reason") or ""),
        )
        ...
        active_conditions.extend(
            [str(x) for x in (release_decision_row.get("release_decision_blockers") or []) if str(x).strip()]
        )
        active_conditions.extend(
            [
                str(x)
                for x in (release_inelig_decision_row.get("release_ineligibility_blockers") or [])
                if str(x).strip()
            ]
        )
        ...
        active_conditions.append(
            f"release_blocker_state:{release_blocker_row.get('release_blocker_state') or 'active'}"
        )
        deduped_active_conditions = sorted({x for x in active_conditions if x})

        required_resolution_path = str(
            release_inelig_decision_row.get("required_resolution_path")
            or release_blocker_row.get("required_resolution")
            or "resolve_all_release_layer_blockers_then_reassess_release_eligibility"
        )

        required_release_authority = str(
            release_inelig_decision_row.get("required_release_authority")
            or release_prohibition_row.get("required_release_authority")
        ...
        required_operator_role = str(
            release_blocker_row.get("required_operator_role")
            or release_inelig_decision_row.get("required_operator_role")
            or release_prohibition_row.get("required_operator_role")
        ...
        rollback_reference = str(
            release_blocker_row.get("rollback_reference")
            or release_prohibition_row.get("rollback_reference")
            or release_inelig_decision_row.get("rollback_reference")
        ...
        record_source_paths: dict[str, Any] = {
            ...
            "model_adjustment_release_blocker_registry_json": source_paths[
                "model_adjustment_release_blocker_registry_json"
            ],
            "active_release_conditions": deduped_active_conditions,
        }

        records.append(
            {
                "release_ineligibility_register_id": f"release-ineligibility-register-{seq:04d}",
                ...
                "release_ineligibility_decision_id": release_inelig_decision_row.get(
                    "release_ineligibility_decision_id"
                ),
                "release_prohibition_id": release_prohibition_row.get("release_prohibition_id"),
                "release_blocker_id": release_blocker_row.get("release_blocker_id"),
                ...
                "active_release_ineligibility_conditions": deduped_active_conditions,
                ...
                "release_ineligibility_notes": (
                    "v4_2_governance_only_release_ineligibility_conditions_remain_active_no_execution_or_model_mutation"
                ),
                "source_paths": record_source_paths,
            }
        )

        state_counts[release_ineligibility_state] = state_counts.get(
            release_ineligibility_state, 0
        ) + 1
        class_counts[release_ineligibility_class] = class_counts.get(
            release_ineligibility_class, 0
        ) + 1

    summary = {
        "total_proposals_in_source": len(proposal_ids),
        "total_release_ineligibility_register_records": len(records),
    ...
    return {
        ...
        "source_versions": {
            ...
            "model_adjustment_release_ineligibility_decisions_version": release_ineligibility_decisions_payload.get(
            ...
            "model_adjustment_release_blocker_registry_version": release_blocker_payload.get(
                "model_adjustment_release_blocker_registry_version"
            ),
        },
        "release_ineligibility_register_summary": summary,
        "release_ineligibility_register": records,
        "source_paths": source_paths,
    }


def render_markdown(payload: dict[str, Any]) -> str:
    summary = payload.get("release_ineligibility_register_summary") or {}
    rows = payload.get("release_ineligibility_register") or []

    lines: list[str] = []
    lines.append("# AI-RISA Controlled Release Ineligibility Register (Slice 1)")
    lines.append("")
    lines.append(f"Generated (UTC): {payload.get('generated_at_utc')}")
    lines.append(
        "Release Ineligibility Register Version: "
        f"{payload.get('model_adjustment_release_ineligibility_register_version')}"
    )
    lines.append("")
    lines.append("## Summary")
    lines.append(f"- Source Proposals: {summary.get('total_proposals_in_source')}")
    lines.append(
        "- Total Ineligibility Register Records: "
        f"{summary.get('total_release_ineligibility_register_records')}"
    )
    ...
    lines.append("")
    lines.append("## Release Ineligibility Register Records")

    if rows:
        for row in rows:
            lines.append(
                f"- {row.get('release_ineligibility_register_id')}: {row.get('proposal_id')} | "
            ...
            lines.append(
                f"  ineligibility_decision={row.get('release_ineligibility_decision_id')} | "
                f"prohibition={row.get('release_prohibition_id')} | blocker={row.get('release_blocker_id')}"
            )
            lines.append(
                f"  state={row.get('release_ineligibility_state')} | "
                f"class={row.get('release_ineligibility_class')}"
            )
            lines.append(f"  reason={row.get('release_ineligibility_reason')}")
            lines.append(
                "  active_release_ineligibility_conditions="
                f"{row.get('active_release_ineligibility_conditions')}"
            )
            lines.append(f"  required_resolution_path={row.get('required_resolution_path')}")
            lines.append(f"  required_release_authority={row.get('required_release_authority')}")
...
def main() -> int:
    ...
    release_ineligibility_decisions_path = Path(args.release_ineligibility_decisions_json)
    release_prohibition_path = Path(args.release_prohibition_matrix_json)
    release_blocker_path = Path(args.release_blocker_registry_json)
    ...
    release_ineligibility_decisions_payload, release_ineligibility_decisions_err = safe_read_json(
        repo_root / release_ineligibility_decisions_path
    )
    release_prohibition_payload, release_prohibition_err = safe_read_json(
        repo_root / release_prohibition_path
    )
    release_blocker_payload, release_blocker_err = safe_read_json(repo_root / release_blocker_path)
    ...
    errors = [
        ...
        ("model-adjustment release ineligibility decisions", release_ineligibility_decisions_err),
        ("model-adjustment release prohibition matrix", release_prohibition_err),
        ("model-adjustment release blocker registry", release_blocker_err),
    ]
    ...
    payloads = [
        ...
        release_ineligibility_decisions_payload,
        release_prohibition_payload,
        release_blocker_payload,
    ]
    ...
    source_paths = {
        ...
        "model_adjustment_release_ineligibility_decisions_json": normalize_path(
            release_ineligibility_decisions_path
        ),
        "model_adjustment_release_prohibition_matrix_json": normalize_path(release_prohibition_path),
        "model_adjustment_release_blocker_registry_json": normalize_path(release_blocker_path),
    }

    payload = build_release_ineligibility_register(
        ...
        release_ineligibility_decisions_payload=release_ineligibility_decisions_payload,
        release_prohibition_payload=release_prohibition_payload,
        release_blocker_payload=release_blocker_payload,
        source_paths=source_paths,
    )
    ...
    payload["source_status"] = {
        ...
        for k, v in [
            ...
            ("model_adjustment_release_ineligibility_decisions_json", release_ineligibility_decisions_err),
            ("model_adjustment_release_prohibition_matrix_json", release_prohibition_err),
            ("model_adjustment_release_blocker_registry_json", release_blocker_err),
        ]
    }
    ...
    print(
        "[STATUS] release_ineligibility_register={total} proposals_covered={covered} "
        "all_covered={all_covered} record_count_matches_expected={match} "
        "deterministic_ordering={ordered} release_ineligibility_state={state}".format(
            total=summary.get("total_release_ineligibility_register_records"),
