...
v6.0-slice-1 — Read-only controlled release resolution-wave-packet-review-session-intake
generator.

Consumes the frozen v5.9 release resolution wave packet review session handoff output
and emits one deterministic intake entry per handoff record.
...
SOURCE_PATHS = {
    "model_adjustment_release_resolution_wave_packet_review_session_handoff_json": (
        "ops/model_adjustments/model_adjustment_release_resolution_wave_packet_review_session_handoff.json"
    )
}

OUTPUT_JSON = os.path.join(
    OPS_DIR,
    "model_adjustment_release_resolution_wave_packet_review_session_intake.json",
)
OUTPUT_MD = os.path.join(
    OPS_DIR,
    "model_adjustment_release_resolution_wave_packet_review_session_intake.md",
)

VERSION = "v6.0-slice-1"
EXPECTED_REVIEW_SESSION_HANDOFF_COUNT = 3
...
REQUIRED_SESSION_HANDOFF_FIELDS = (
...
def normalize_non_empty_string(value, field_name: str, index: int) -> str:
    if not isinstance(value, str):
        raise ValueError(
            f"session-handoff record[{index}] field {field_name!r} must be str; fail-closed"
        )
    normalized = value.strip()
    if not normalized:
        raise ValueError(
            f"session-handoff record[{index}] field {field_name!r} must be non-empty; fail-closed"
        )
    return normalized
...
def normalize_non_negative_int(value, field_name: str, index: int) -> int:
    if isinstance(value, bool):
        raise ValueError(
            f"session-handoff record[{index}] field {field_name!r} must be int, not bool; fail-closed"
        )
    try:
        normalized = int(value)
    except (TypeError, ValueError) as exc:
        raise ValueError(
            f"session-handoff record[{index}] field {field_name!r} malformed int={value!r}; fail-closed"
        ) from exc
    if normalized < 0:
        raise ValueError(
            f"session-handoff record[{index}] field {field_name!r} must be >= 0; fail-closed"
        )
    return normalized
...
def normalize_bool(value, field_name: str, index: int) -> bool:
    if not isinstance(value, bool):
        raise ValueError(
            f"session-handoff record[{index}] field {field_name!r} must be bool; fail-closed"
        )
    return value
...
def normalize_string_list(value, field_name: str, index: int) -> list:
    if not isinstance(value, list):
        raise ValueError(
            f"session-handoff record[{index}] field {field_name!r} must be list; fail-closed"
        )
    normalized_values = []
    for item_index, item in enumerate(value):
        if not isinstance(item, str):
            raise ValueError(
                f"session-handoff record[{index}] field {field_name!r} non-str item at {item_index}; fail-closed"
            )
        item_value = item.strip()
        if not item_value:
            raise ValueError(
                f"session-handoff record[{index}] field {field_name!r} empty item at {item_index}; fail-closed"
            )
...
def validate_upstream_payload(source_data: dict) -> None:
    if not isinstance(source_data, dict):
        raise ValueError("upstream session handoff payload is not a dict; fail-closed")

    version = source_data.get(
        "model_adjustment_release_resolution_wave_packet_review_session_handoff_version"
    )
    if not isinstance(version, str) or not version.strip():
        raise ValueError("upstream session handoff version missing or malformed; fail-closed")

    records = source_data.get("release_resolution_wave_packet_review_session_handoff")
    if not isinstance(records, list):
        raise ValueError("upstream release_resolution_wave_packet_review_session_handoff missing or malformed; fail-closed")


def validate_session_handoff_record(record: dict, index: int) -> None:
    if not isinstance(record, dict):
        raise ValueError(f"session-handoff record[{index}] malformed type={type(record).__name__}; fail-closed")

    for field in REQUIRED_SESSION_HANDOFF_FIELDS:
        if field not in record:
            raise ValueError(f"session-handoff record[{index}] missing required field {field!r}; fail-closed")
    ...
    if record["wave_type"] not in (
        ...
        raise ValueError(
            f"session-handoff record[{index}] unknown wave_type={record['wave_type']!r}; fail-closed"
        )

    if record["review_lane"] not in REVIEW_LANE_ORDER:
        raise ValueError(
            f"session-handoff record[{index}] unknown review_lane={record['review_lane']!r}; fail-closed"
        )


def sorted_session_handoff_records(records: list) -> list:
    return sorted(
        records,
        key=lambda record: (
            REVIEW_LANE_ORDER[record["review_lane"]],
            record["review_session_handoff_priority"],
            record["wave_rank"],
            record["packet_priority"],
            record["checklist_priority"],
            record["resolution_wave_packet_review_session_handoff_id"],
            record["source_resolution_wave_packet_review_session_brief_id"],
        ),
    )


def build_intake_records(records: list) -> list:
    for index, record in enumerate(records):
        validate_session_handoff_record(record, index)

    output = []
    for index, record in enumerate(sorted_session_handoff_records(records), start=1):
        output.append(
            {
                "resolution_wave_packet_review_session_intake_id": (
                    f"resolution-wave-packet-review-session-intake-{index:04d}"
                ),
                "source_resolution_wave_packet_review_session_handoff_id": record[
                    "resolution_wave_packet_review_session_handoff_id"
                ],
                ...
                "source_resolution_wave_packet_id": record["source_resolution_wave_packet_id"],
                ...
                "review_session_handoff_priority": record["review_session_handoff_priority"],
                "review_lane": record["review_lane"],
                "review_session_intake_priority": record["review_session_handoff_priority"],
                "intake_position": index,
                ...
                "review_session_intake_notes": (
                    "v6_0_read_only_review_session_intake_projection_of_v5_9_session_handoff"
...
def validate_intake(records: list, session_handoff_records: list) -> None:
    sorted_handoffs = sorted_session_handoff_records(session_handoff_records)

    if len(records) != len(sorted_handoffs):
        raise ValueError("session intake count mismatch against sorted handoff records; fail-closed")

    source_handoff_ids = [
        record["source_resolution_wave_packet_review_session_handoff_id"] for record in records
    ]
    expected_handoff_order = [
        record.get("resolution_wave_packet_review_session_handoff_id", "")
        for record in sorted_handoffs
    ]
    if source_handoff_ids != expected_handoff_order:
        raise ValueError("session intake records are not in deterministic handoff sort order; fail-closed")

    intake_ids = [record["resolution_wave_packet_review_session_intake_id"] for record in records]
    if len(intake_ids) != len(set(intake_ids)):
        raise ValueError("duplicate resolution_wave_packet_review_session_intake_id in output; fail-closed")

    if len(source_handoff_ids) != len(set(source_handoff_ids)):
        raise ValueError("duplicate source_resolution_wave_packet_review_session_handoff_id in intake; fail-closed")

    upstream_handoff_ids = [
        record.get("resolution_wave_packet_review_session_handoff_id", "")
        for record in session_handoff_records
    ]
    if len(upstream_handoff_ids) != EXPECTED_REVIEW_SESSION_HANDOFF_COUNT:
        raise ValueError(
            "upstream session handoff count does not match EXPECTED_REVIEW_SESSION_HANDOFF_COUNT; fail-closed"
        )

    if set(source_handoff_ids) != set(upstream_handoff_ids):
        raise ValueError("session intake coverage does not match upstream handoff records; fail-closed")

    for record in records:
        if record["review_session_intake_priority"] != record["review_session_handoff_priority"]:
            raise ValueError("review_session_intake_priority drift detected; fail-closed")
...
def build_summary(records: list, session_handoff_records: list) -> dict:
    lane_counts = {
    ...
    return {
        "upstream_session_handoff_count": len(session_handoff_records),
        "total_review_session_intake_records": len(records),
...
def build_source_versions(source_data: dict) -> dict:
    return {
        "model_adjustment_release_resolution_wave_packet_review_session_handoff_version": source_data.get(
            "model_adjustment_release_resolution_wave_packet_review_session_handoff_version", ""
        )
    }
...
def build_markdown_lines(records: list, summary: dict, source_versions: dict, generated_at: str) -> list:
    lines = [
        "# Model Adjustment Release Resolution Wave Packet Review Session Intake",
        "",
        f"**Version**: {VERSION}",
    ...
    lines.append("")
    for record in records:
        lines.append(f"## {record['resolution_wave_packet_review_session_intake_id']}")
        lines.append("")
        lines.append(
            f"- source_resolution_wave_packet_review_session_handoff_id: {record['source_resolution_wave_packet_review_session_handoff_id']}"
        )
        lines.append(
        ...
        lines.append(f"- review_session_intake_priority: {record['review_session_intake_priority']}")
        lines.append(f"- intake_position: {record['intake_position']}")
...
def main():
    ...
    source_data, errors[
        "model_adjustment_release_resolution_wave_packet_review_session_handoff_json"
    ] = load_source("model_adjustment_release_resolution_wave_packet_review_session_handoff_json")
    ...
    session_handoff_records = source_data.get(
        "release_resolution_wave_packet_review_session_handoff", []
    )
    records = build_intake_records(session_handoff_records)
    validate_intake(records, session_handoff_records)
    summary = build_summary(records, session_handoff_records)
    ...
    payload = {
        ...
        "model_adjustment_release_resolution_wave_packet_review_session_intake_version": VERSION,
        "release_resolution_wave_packet_review_session_intake_summary": summary,
        "release_resolution_wave_packet_review_session_intake": records,
    ...
    print(
        "[STATUS] "
        f"review_session_intake_records={summary['total_review_session_intake_records']} "
        f"upstream_session_handoff_count={summary['upstream_session_handoff_count']} "
