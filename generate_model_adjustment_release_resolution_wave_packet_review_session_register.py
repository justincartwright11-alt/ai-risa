#!/usr/bin/env python3
"""
v6.2: Model Adjustment Release Resolution Wave Packet Review Session Register Generator

Pure downstream projection of review-session-receipt into operator-register ledgering layer.
Consumes: model_adjustment_release_resolution_wave_packet_review_session_receipt.json (v6.1)
Produces: model_adjustment_release_resolution_wave_packet_review_session_register.json (v6.2)
          model_adjustment_release_resolution_wave_packet_review_session_register.md (v6.2)

Behavior:
- One register entry per upstream receipt entry
- Preserves all upstream priority, lane, posture fields exactly (pass-through)
- Adds register-specific projection fields only (register_position, register_priority, register_id)
- Deterministic ordering stable on receipt_id / intake_id
...
SOURCE_PATHS = {
    "receipt": "ops/model_adjustments/model_adjustment_release_resolution_wave_packet_review_session_receipt.json",
}

OUTPUT_PATHS = {
    "json": "ops/model_adjustments/model_adjustment_release_resolution_wave_packet_review_session_register.json",
    "md": "ops/model_adjustments/model_adjustment_release_resolution_wave_packet_review_session_register.md",
}
...
def validate_upstream_payload(payload, path):
    ...
    version = payload.get("model_adjustment_release_resolution_wave_packet_review_session_receipt_version")
    if version != "v6.1-slice-1":
        raise ValueError(f"Expected receipt version 'v6.1-slice-1', got {repr(version)}")

    records = payload.get("release_resolution_wave_packet_review_session_receipt", [])
    if not isinstance(records, list):
        raise ValueError(
            "release_resolution_wave_packet_review_session_receipt is not a list: "
            f"{type(records).__name__}"
        )
    if len(records) == 0:
        raise ValueError("No receipt records found in upstream payload")
    return records


def build_register_records(receipt_records):
    register_records = []

    for idx, receipt_record in enumerate(receipt_records):
        try:
            receipt_id = normalize_string(
                receipt_record.get("resolution_wave_packet_review_session_receipt_id"),
                "receipt_id",
            )
            intake_id = normalize_string(
                receipt_record.get("source_resolution_wave_packet_review_session_intake_id"),
                "intake_id",
            )
            handoff_id = normalize_string(
                receipt_record.get("source_resolution_wave_packet_review_session_handoff_id"),
                "handoff_id",
            )
            brief_id = normalize_string(
                receipt_record.get("source_resolution_wave_packet_review_session_brief_id"),
                "brief_id",
            )
            pack_id = normalize_string(
                receipt_record.get("source_resolution_wave_packet_review_session_pack_id"),
                "pack_id",
            )
            agenda_id = normalize_string(
                receipt_record.get("source_resolution_wave_packet_review_agenda_id"),
                "agenda_id",
            )
            docket_id = normalize_string(
                receipt_record.get("source_resolution_wave_packet_review_docket_id"),
                "docket_id",
            )
            board_id = normalize_string(
                receipt_record.get("source_resolution_wave_packet_review_board_id"),
                "board_id",
            )
            checklist_id = normalize_string(
                receipt_record.get("source_resolution_wave_packet_checklist_id"),
                "checklist_id",
            )
            packet_id = normalize_string(
                receipt_record.get("source_resolution_wave_packet_id"),
                "packet_id",
            )
            wave_id = normalize_string(
                receipt_record.get("source_resolution_wave_id"),
                "wave_id",
            )

            wave_rank = normalize_int(receipt_record.get("wave_rank"), "wave_rank")
            wave_type = normalize_string(receipt_record.get("wave_type"), "wave_type")
            packet_priority = normalize_string(receipt_record.get("packet_priority"), "packet_priority")
            checklist_priority = normalize_string(receipt_record.get("checklist_priority"), "checklist_priority")
            review_board_priority = normalize_string(
                receipt_record.get("review_board_priority"), "review_board_priority"
            )
            review_docket_priority = normalize_string(
                receipt_record.get("review_docket_priority"), "review_docket_priority"
            )
            review_agenda_priority = normalize_string(
                receipt_record.get("review_agenda_priority"), "review_agenda_priority"
            )
            review_session_pack_priority = normalize_string(
                receipt_record.get("review_session_pack_priority"), "review_session_pack_priority"
            )
            review_session_brief_priority = normalize_string(
                receipt_record.get("review_session_brief_priority"), "review_session_brief_priority"
            )
            review_session_handoff_priority = normalize_string(
                receipt_record.get("review_session_handoff_priority"), "review_session_handoff_priority"
            )
            review_session_intake_priority = normalize_string(
                receipt_record.get("review_session_intake_priority"), "review_session_intake_priority"
            )
            review_session_receipt_priority = normalize_string(
                receipt_record.get("review_session_receipt_priority"), "review_session_receipt_priority"
            )
            review_lane = normalize_string(receipt_record.get("review_lane"), "review_lane")
            terminal_posture = normalize_string(receipt_record.get("terminal_posture"), "terminal_posture")

            member_cluster_ids = normalize_list(
                receipt_record.get("member_cluster_ids", []), "member_cluster_ids"
            )
            member_dependency_ids = normalize_list(
                receipt_record.get("member_dependency_ids", []), "member_dependency_ids"
            )
            member_source_refs = normalize_list(
                receipt_record.get("member_source_refs", []), "member_source_refs"
            )
            affected_proposal_ids = normalize_list(
                receipt_record.get("affected_proposal_ids", []), "affected_proposal_ids"
            )
            affected_queue_ids = normalize_list(
                receipt_record.get("affected_queue_ids", []), "affected_queue_ids"
            )

            affected_record_count = normalize_int(
                receipt_record.get("affected_record_count"), "affected_record_count"
            )
            cluster_count = normalize_int(receipt_record.get("cluster_count"), "cluster_count")
            dependency_count = normalize_int(receipt_record.get("dependency_count"), "dependency_count")
            has_prohibition_path = normalize_bool(
                receipt_record.get("has_prohibition_path"), "has_prohibition_path"
            )
            has_blocker_path = normalize_bool(
                receipt_record.get("has_blocker_path"), "has_blocker_path"
            )
        except ValueError as e:
            raise ValueError(f"Validation error in receipt record {idx}: {e}")

        register_records.append(
            {
                "resolution_wave_packet_review_session_register_id": (
                    f"resolution-wave-packet-review-session-register-{idx + 1:04d}"
                ),
                ...
                "review_session_register_priority": review_session_receipt_priority,
                "register_position": idx + 1,
    ...
    for idx, rec in enumerate(register_records):
        rec["register_position"] = idx + 1
    return register_records


def generate_markdown(payload, records):
    md = f"""# Model Adjustment Release Resolution Wave Packet Review Session Register

**Version**: v6.2-slice-1
...
| Field | Value |
|---|---|
| upstream_session_receipt_count | {payload['upstream_session_receipt_count']} |
| total_review_session_register_records | {len(records)} |
...
| Source | Version |
|---|---|
| model_adjustment_release_resolution_wave_packet_review_session_receipt_version | {payload['upstream_session_receipt_version']} |

"""

    for rec in records:
        md += f"""## {rec['resolution_wave_packet_review_session_register_id']}
...
- register_position: {rec['register_position']}
...
def main():
    try:
        ...
        if not os.path.exists(receipt_path):
            raise FileNotFoundError(f"Upstream receipt JSON not found: {receipt_path}")

        with open(receipt_path, "r") as f:
            receipt_payload = json.load(f)

        receipt_records = validate_upstream_payload(receipt_payload, receipt_path)
        register_records = build_register_records(receipt_records)

        coverage_reconciled = len(receipt_records) == len(register_records)
        is_sorted = all(
            register_records[i]["register_position"] <= register_records[i + 1]["register_position"]
            for i in range(len(register_records) - 1)
        )
        ...
        output_payload = {
            "model_adjustment_release_resolution_wave_packet_review_session_register_version": "v6.2-slice-1",
            "generated_at_utc": datetime.datetime.now(datetime.timezone.utc).isoformat(),
            "upstream_session_receipt_count": len(receipt_records),
            "upstream_session_receipt_version": receipt_payload.get(
                "model_adjustment_release_resolution_wave_packet_review_session_receipt_version"
            ),
            "release_resolution_wave_packet_review_session_register": register_records,
            "coverage_reconciled": coverage_reconciled,
            "deterministic_ordering": is_sorted,
            "review_lane_counts": lane_counts,
        }

        markdown_output = generate_markdown(output_payload, register_records)
        ...
        print(
            f"[STATUS] review_session_register_records={len(register_records)} "
            f"upstream_session_receipt_count={len(receipt_records)} "
