...
OUTPUT_JSON = "ops/model_adjustments/model_adjustment_release_resolution_wave_packet_review_session_execution_audit.json"
OUTPUT_MD = "ops/model_adjustments/model_adjustment_release_resolution_wave_packet_review_session_execution_audit.md"
...
def main():
    ...
    if not batch_records:
        output = {
            "record_type": "model_adjustment_release_resolution_wave_packet_review_session_execution_audit",
            "source_files": [INTAKE_FILE, QUEUE_FILE, DISPATCH_FILE, BATCH_FILE],
            "source_record_counts": {
                "execution_intake": len(intake_records),
                "execution_queue": len(queue_records),
                "execution_dispatch": len(dispatch_records),
                "execution_batch": len(batch_records),
            },
            "record_count": 0,
        ...
        for i, (intake, queue, dispatch, batch) in enumerate(
            ...
            suffix_ok = (
                intake["execution_intake_id"][-4:] == queue["execution_queue_id"][-4:] ==
                dispatch["execution_dispatch_id"][-4:] == batch["execution_batch_id"][-4:]
            )
            ...
            lineage_ok = (
                batch["execution_dispatch_id"] == dispatch["execution_dispatch_id"] and
                dispatch["execution_queue_id"] == queue["execution_queue_id"] and
                queue["execution_intake_id"] == intake["execution_intake_id"]
            )
            exact_once_lineage = exact_once_lineage and lineage_ok
            audit_record = {
                "execution_audit_id": f"resolution-wave-packet-review-session-execution-audit-{i:04d}",
                "audit_position": i,
                "execution_intake_id": intake["execution_intake_id"],
                "execution_queue_id": queue["execution_queue_id"],
                ...
                "lineage_source_layer": "execution_batch",
                "lineage_source_file": BATCH_FILE,
                "lineage_source_record_id": batch["execution_batch_id"],
            }
