"""
Button 2 Customer PDF - Phase 6 Controlled Export Smoke v1

Evidence-only smoke proof that controlled export preview remains preview-only,
approval-gated, server-path-bound, and non-operational.
"""
...
def _valid_ctx(**overrides):
    base = {
        ...
        "fight_id": "fight_phase6_controlled_export_smoke_001",
        "source_context_kind": "button1_dossier_handoff",
        "source_ingest_mode": "preview_only",
        "handoff_summary_preview": "Phase 6 controlled export smoke status surface.",
        "hierarchy_markers": [
...
def test_controlled_export_eligible_only_when_customer_ready_met():
...
def test_controlled_export_not_eligible_when_customer_ready_missing():
...
def test_operator_approval_required_gate_text_present():
    html = build_button2_report_html(_valid_ctx())["html_content"]
    assert "Gate: operator_approval_required" in html


def test_server_derived_output_path_required_policy_present():
    html = build_button2_report_html(_valid_ctx())["html_content"]
    assert "Output path policy: server_derived_output_path_required" in html


def test_no_export_execution_controls_or_delivery_automation():
    html = build_button2_report_html(_valid_ctx())["html_content"].lower()
    assert "deliver now" not in html
    assert "export now" not in html
    assert "download now" not in html
...
def test_no_approval_bypass_or_user_supplied_path_controls():
    html = build_button2_report_html(_valid_ctx())["html_content"].lower()
    assert "user-supplied path" not in html
    assert "choose path" not in html
    assert "custom location" not in html


def test_no_new_unsafe_write_or_dashboard_behavior_changes():
