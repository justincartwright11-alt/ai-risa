"""
Button 2 Customer PDF - Phase 4 Header/Footer/Watermark Rendering v1

Purpose:
- Validate first narrow header/footer/watermark rendering polish step is CSS-only and contract-safe.
...
def _valid_ctx(**overrides):
    base = {
        ...
        "fight_id": "fight_phase4_hfw_rendering_001",
        "source_context_kind": "button1_dossier_handoff",
        "source_ingest_mode": "preview_only",
        "handoff_summary_preview": "Phase 4 header/footer/watermark rendering polish remains metadata-locked.",
        "hierarchy_markers": [
...
def test_header_footer_watermark_render_facing_css_present():
...
def test_running_header_footer_layout_rules_represented():
    html = build_button2_report_html(_valid_ctx())["html_content"]
...
def test_page_number_styling_rules_represented():
    html = build_button2_report_html(_valid_ctx())["html_content"]
    assert ".running-footer .page-number," in html
    assert ".page-number-label {" in html
    assert "font-variant-numeric: tabular-nums;" in html


def test_confidentiality_and_status_label_styling_represented():
    html = build_button2_report_html(_valid_ctx())["html_content"]
...
def test_watermark_styling_rules_represented():
    html = build_button2_report_html(_valid_ctx())["html_content"]
...
def test_locked_header_footer_watermark_metadata_remains_intact():
    html = build_button2_report_html(_valid_ctx())["html_content"]
