"""
Button 2 Customer PDF - Phase 4 Typography/Layout Polish v1

Purpose:
- Validate first narrow Phase 4 renderer-polish step is typography/layout CSS only.
...
def _valid_ctx(**overrides):
    base = {
        ...
        "fight_id": "fight_phase4_typography_layout_001",
        "source_context_kind": "button1_dossier_handoff",
        "source_ingest_mode": "preview_only",
        "handoff_summary_preview": "Phase 4 Typography/Layout Polish: readability pass only.",
        "hierarchy_markers": [
...
def test_layout_spacing_rules_represented():
    html = build_button2_report_html(_valid_ctx())["html_content"]
...
def test_section_readability_improves_via_css_only():
    html = build_button2_report_html(_valid_ctx())["html_content"]
    assert "h2   { margin-top: 1.35em; margin-bottom: 0.45em;" in html
    assert "pre  { white-space: pre-wrap; word-break: break-word; margin-top: 0.4em; }" in html
    assert 'id="button2-hierarchy-metadata"' in html


def test_no_html_data_contract_change():
    html = build_button2_report_html(_valid_ctx())["html_content"]
    assert 'id="button2-hierarchy-metadata"' in html
...
def test_no_proof_stack_contract_change_in_output_markers():
    html = build_button2_report_html(_valid_ctx())["html_content"]
...
def test_no_chart_rendering_introduced():
...
def test_no_delivery_or_certification_controls_added():
    html = build_button2_report_html(_valid_ctx())["html_content"].lower()
...
def test_no_approval_output_filewrite_dashboard_changes():
