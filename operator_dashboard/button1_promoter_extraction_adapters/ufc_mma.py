"""
UFC MMA full-card extraction adapter.

Approved sources: ufc.com/event/, ufcstats.com (under registry governance)
...
Current production status for UFC 300:
  ...
  ufc.com/event/ and set expected_matchup_count.
"""
...
class UFCMMAAdapter(BaseEventCardExtractionAdapter):
    """Extraction adapter for UFC official event cards."""
    ...
    # Typical UFC PPV full card: ~13 bouts (main + prelims + early prelims).
    # Used only when expected_matchup_count is not set in the feed row.
    _TYPICAL_FULL_CARD_MIN = 10
    ...
    def extract(self, event_row: Dict[str, Any]) -> ExtractionResult:
        ...
        if completeness == "headline_only":
            ...
            diagnostics = (
                ...
                "Full card extraction from ufc.com is not yet implemented. "
                "To upgrade: populate matchups[] in the feed with all verified bouts from "
                "the approved UFC event page and set expected_matchup_count."
            )
            ...
            diagnostics = (
                f"{len(matchups)} matchup(s) found but expected ~{known} for a full UFC card. "
