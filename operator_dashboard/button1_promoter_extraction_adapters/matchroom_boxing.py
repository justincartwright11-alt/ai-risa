"""
Matchroom Boxing full-card extraction adapter.

Approved sources: matchroomboxing.com, queensberry.com, topleftboxing.com, nolimitboxing.com
...
Current production status for Joshua vs Dubois:
  ...
  matchroomboxing.com and set expected_matchup_count.
"""
...
class MatchroomBoxingAdapter(BaseEventCardExtractionAdapter):
    """Extraction adapter for Matchroom Boxing approved event cards."""
    ...
    # Typical Matchroom main-card minimum for a major event.
    # Used only when expected_matchup_count is not set in the feed row.
    _TYPICAL_FULL_CARD_MIN = 6
    ...
    def extract(self, event_row: Dict[str, Any]) -> ExtractionResult:
        ...
        if completeness == "headline_only":
            ...
            diagnostics = (
                ...
                "Full card extraction from matchroomboxing.com is not yet implemented. "
                "To upgrade: populate matchups[] in the feed with all verified bouts from "
                "the approved source URL and set expected_matchup_count."
            )
            ...
            diagnostics = (
                f"{len(matchups)} matchup(s) found but expected ~{known} for a full card. "
