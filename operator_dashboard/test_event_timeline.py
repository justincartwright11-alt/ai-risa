...
class TestEventTimeline(unittest.TestCase):
    ...
    def test_timeline_hit(self):
        resp = self.client.get('/api/queue/event/song_vs_figueiredo/timeline')
        ...
        self.assertIn('recent_ledger_entries', data)
        self.assertLessEqual(len(data['recent_ledger_entries']), 5)
        self.assertTrue(any('Timeline' in e['user_message'] for e in data['recent_ledger_entries']))
        self.assertIn('recommendation', data)
        self.assertIn('errors', data)

    def test_timeline_miss(self):
        resp = self.client.get('/api/queue/event/zzz/timeline')
        self.assertEqual(resp.status_code, 200)
        data = json.loads(resp.data)
        self.assertTrue(data['ok'])
