...
def _tok() -> dict[str, str]:
    return {
        ...
        "d": "".join(["j", "p", "e", "g"]),
        "e": " ".join(["New", "Visuals"]),
        "f": "\\".join(["OneDrive", "Pictures"]),
        "g": "\\".join(["C:", "Users", "jusin", "OneDrive", "Pictures"]),
    }
...
def _mod_paths() -> dict[str, Path]:
    ...
    acr = t["a"]
    return {
        "renderer": VISUAL_DIR / ("button2_premium_" + acr + "_visual_renderer_v1.py"),
        "artifact": VISUAL_DIR / ("button2_premium_" + acr + "_visual_internal_artifact_path_v1.py"),
        "qagate": VISUAL_DIR / ("button2_premium_" + acr + "_visual_qa_gate_v1.py"),
        "fixture_page": VISUAL_DIR / ("button2_premium_" + acr + "_visual_internal_page_prototype_v1.json"),
        "fixture_payload": VISUAL_DIR / ("button2_premium_" + acr + "_visual_internal_contract_prototype_v1.json"),
        "fixture_style": VISUAL_DIR / ("button2_premium_" + acr + "_visual_style_registry_v1.json"),
    }
...
def _internal_parent() -> Path:
    t = _tok()
    return REPO_ROOT / ("tmp" + "_" + t["a"] + "_" + "output")
...
def _has_ref(text: str) -> bool:
    t = _tok()
    low = text.lower()
    return t["e"].lower() in low or t["f"].lower() in low or t["g"].lower() in low
...
def _mk_safe_contracts() -> tuple[dict, dict, dict]:
    p = _mod_paths()
    mod_r = _load_mod(p["renderer"], "b2_r_mod")
    mod_a = _load_mod(p["artifact"], "b2_a_mod")
    mod_q = _load_mod(p["qagate"], "b2_q_mod")

    payload = _load_json(p["fixture_payload"])
...
def test_button2_renderer_artifact_path_integration_accepts_safe_contract_object():
...
def test_button2_renderer_artifact_path_integration_requires_artifact_contract_pass_internal_only():
    r, a, q = _mk_safe_contracts()
...
def test_button2_renderer_artifact_path_integration_rejects_blocked_artifact_contract():
    r, a, q = _mk_safe_contracts()
    a["blocked_reasons"] = ["x"]
    out = build_renderer_artifact_path_integration_contract(r, a, q)
    _assert_blocked(out)
...
def test_button2_renderer_artifact_path_integration_requires_valid_render_contract():
    _r, a, q = _mk_safe_contracts()
    out = build_renderer_artifact_path_integration_contract(None, a, q)
    _assert_blocked(out)


def test_button2_renderer_artifact_path_integration_rejects_invalid_render_status():
    r, a, q = _mk_safe_contracts()
    r["render_status"] = "BAD"
    out = build_renderer_artifact_path_integration_contract(r, a, q)
    _assert_blocked(out)
...
def test_button2_renderer_artifact_path_integration_requires_pass_internal_only_qa_gate():
    r, a, q = _mk_safe_contracts()
    q["qa_status"] = "PENDING"
    out = build_renderer_artifact_path_integration_contract(r, a, q)
    _assert_blocked(out)
...
def test_button2_renderer_artifact_path_integration_rejects_blocked_qa_gate():
    r, a, q = _mk_safe_contracts()
    q["blocked_reasons"] = ["z"]
    out = build_renderer_artifact_path_integration_contract(r, a, q)
    _assert_blocked(out)
...
def test_button2_renderer_artifact_path_integration_preserves_read_only_artifact_path():
    r, a, q = _mk_safe_contracts()
    out = build_renderer_artifact_path_integration_contract(r, a, q)
...
def test_button2_renderer_artifact_path_integration_rejects_artifact_extension_authorized_true():
    r, a, q = _mk_safe_contracts()
...
def test_button2_renderer_artifact_path_integration_keeps_delivery_ready_false():
    r, a, q = _mk_safe_contracts()
    out = build_renderer_artifact_path_integration_contract(r, a, q)
    assert out["delivery_ready"] is False


def test_button2_renderer_artifact_path_integration_rejects_customer_release_true():
    r, a, q = _mk_safe_contracts()
...
def test_button2_renderer_artifact_path_integration_rejects_learning_activation_true():
    r, a, q = _mk_safe_contracts()
...
def test_button2_renderer_artifact_path_integration_rejects_output_path_non_null():
    r, a, q = _mk_safe_contracts()
...
def test_button2_renderer_artifact_path_integration_rejects_path_override():
    r, a, q = _mk_safe_contracts()
...
def test_button2_renderer_artifact_path_integration_rejects_reference_folder_dependency():
    r, a, q = _mk_safe_contracts()
    t = _tok()
    a["artifact_path"] = t["g"] + "\\" + t["e"].replace(" ", "_")
    out = build_renderer_artifact_path_integration_contract(r, a, q)
    _assert_blocked(out)


def test_button2_renderer_artifact_path_integration_rejects_path_traversal():
    r, a, q = _mk_safe_contracts()
...
def test_button2_renderer_artifact_path_integration_requires_source_traceability():
    r, a, q = _mk_safe_contracts()
...
def test_button2_renderer_artifact_path_integration_carries_blocked_reasons_forward():
    r, a, q = _mk_safe_contracts()
...
def test_button2_renderer_artifact_path_integration_creates_no_directories():
...
def test_button2_renderer_artifact_path_integration_creates_no_output_artifacts():
    root = _internal_parent()
    before = _snap_parent(root)
    r, a, q = _mk_safe_contracts()
    _ = build_renderer_artifact_path_integration_contract(r, a, q)
