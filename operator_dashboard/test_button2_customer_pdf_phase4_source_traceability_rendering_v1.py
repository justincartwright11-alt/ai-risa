"""
Button 2 Customer PDF - Phase 4 Source-Traceability Rendering v1

Purpose:
- Validate source-traceability rendering polish is CSS-only and contract-safe.
...
def _valid_ctx(**overrides):
    base = {
        ...
        "fight_id": "fight_phase4_source_traceability_001",
        "source_context_kind": "button1_dossier_handoff",
        "source_ingest_mode": "preview_only",
        "handoff_summary_preview": "Phase 4 source-traceability rendering polish remains metadata-locked.",
        "hierarchy_markers": [
...
def test_source_traceability_render_facing_css_present():
...
def test_citation_and_source_label_styling_represented():
    html = build_button2_report_html(_valid_ctx())["html_content"]
...
def test_official_research_operator_ai_risa_source_class_styling_represented():
    html = build_button2_report_html(_valid_ctx())["html_content"]
...
def test_source_footer_layout_and_traceability_qa_styling_represented():
    html = build_button2_report_html(_valid_ctx())["html_content"]
...
def test_locked_source_traceability_metadata_remains_intact():
    ...
    assert payload["sources"][0]["source_type"] == "official"
    assert payload["sources"][1]["source_type"] == "research"
    assert payload["sources"][2]["source_type"] == "operator"
