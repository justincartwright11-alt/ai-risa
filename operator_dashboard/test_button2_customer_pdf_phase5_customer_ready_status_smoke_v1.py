"""
Button 2 Customer PDF - Phase 5 Customer-Ready Status Smoke v1

Evidence-only smoke proof that customer-ready preview status remains proof-derived,
...
def _valid_ctx(**overrides):
    base = {
        ...
        "fight_id": "fight_phase5_customer_ready_smoke_001",
        "source_context_kind": "button1_dossier_handoff",
        "source_ingest_mode": "preview_only",
        "handoff_summary_preview": "Phase 5 customer-ready smoke status surface.",
        "hierarchy_markers": [
...
def test_customer_ready_recommended_only_when_proof_and_certification_met():
...
def test_customer_ready_not_ready_when_proof_or_certification_missing():
...
def test_operator_approval_gate_text_remains_present():
    html = build_button2_report_html(_valid_ctx())["html_content"]
    assert "Gate: operator_approval_required" in html


def test_status_creates_no_operational_controls_or_bypass():
...
def test_no_file_write_or_delivery_behavior_change():
