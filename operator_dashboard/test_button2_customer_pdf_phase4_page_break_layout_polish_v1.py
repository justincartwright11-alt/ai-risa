"""
Button 2 Customer PDF - Phase 4 Page-Break Layout Polish v1

Purpose:
- Validate first narrow page-break/layout polish step is CSS-only and contract-safe.

Governance:
- No renderer rewrite
- No chart rendering behavior changes
- No approval/output/file-write/dashboard/delivery/certification changes
"""
...
def _valid_ctx(**overrides):
    base = {
        ...
        "fight_id": "fight_phase4_page_break_layout_001",
        "source_context_kind": "button1_dossier_handoff",
        "source_ingest_mode": "preview_only",
        "handoff_summary_preview": "Phase 4 page-break/layout polish keeps metadata contracts stable.",
        "hierarchy_markers": [
...
def test_page_break_css_rules_present():
...
def test_section_block_keep_together_rules_present():
    html = build_button2_report_html(_valid_ctx())["html_content"]
...
def test_widow_orphan_prevention_css_represented():
    html = build_button2_report_html(_valid_ctx())["html_content"]
...
def test_existing_section_block_metadata_remains_intact():
    html = build_button2_report_html(_valid_ctx())["html_content"]
...
def test_existing_page_break_metadata_remains_intact():
    html = build_button2_report_html(_valid_ctx())["html_content"]
    metadata = _extract_metadata_json(html, "button2-page-breaks-metadata")
    assert metadata["validation_status"] == "valid"
