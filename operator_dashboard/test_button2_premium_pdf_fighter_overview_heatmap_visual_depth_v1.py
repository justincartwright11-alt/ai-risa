"""Button 2 premium PDF fighter overview + heatmap visual depth tests v1."""
...
def _assert_visual_depth(data: dict, reader: PdfReader, fighter_a: str, fighter_b: str):
    ...
    # Ensure dashboard section doesn't regress into a compressed metadata dump marker.
    assert "template renderer profile" not in lower_text
    assert "raw ingest mode" not in lower_text

    assert fighter_a in full_text
    assert fighter_b in full_text

    # Open route should be present and callable.
...
def test_rico_visual_depth_v1(monkeypatch, tmp_path):
    ...
    _assert_visual_depth(data, reader, "Rico Verhoeven", "Tariq Osaro")


def test_anthony_visual_depth_v1(monkeypatch, tmp_path):
    data, reader = _generate_pdf(
        tmp_path,
        "Anthony Joshua",
        "Daniel Dubois",
    ...
    _assert_visual_depth(data, reader, "Anthony Joshua", "Daniel Dubois")


def test_alex_visual_depth_v1(monkeypatch, tmp_path):
    data, reader = _generate_pdf(
        tmp_path,
        "Alex Pereira",
        "Jiri Prochazka",
    ...
    _assert_visual_depth(data, reader, "Alex Pereira", "Jiri Prochazka")


def test_dashboard_library_routes_live_v1(tmp_path):
    app.config["TESTING"] = True
    os.environ["BUTTON2_PDF_OUTPUT_ROOT"] = str(tmp_path)
