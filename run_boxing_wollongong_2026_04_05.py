...
def predict_winner(matchup):
    ...
    # Simple scoring: undefeated, recent form, power, age
    score1 = (f1_data['last_5'].count('W') > f2_data['last_5'].count('W')) + (f1_data['age'] < f2_data['age'])
    score2 = (f2_data['last_5'].count('W') > f1_data['last_5'].count('W')) + (f2_data['age'] < f1_data['age'])
    if score1 > score2:
        winner = f1
        ...
        edge = "Even"
    profile = {
        "decision_structure": "Aggressive, high-volume exchanges",
...
def main():
    event = load_json("C:/ai_risa_data/events/boxing_wollongong_2026_04_05.json")
    results = {}
    ...
    save_json("C:/ai_risa_data/reports/boxing_wollongong_2026_04_05_summary.json", summary)
