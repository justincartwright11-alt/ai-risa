"""Runtime proof for Button 2 Jbalia template sample renderer hard-bind v1."""
...
SLICE_DIR = Path(
    r"C:\Users\jusin\OneDrive\Documents\Custom Office Templates\ops\release_checks\button2_jbalia_template_sample_renderer_hard_bind_v1"
)
VISUAL_DIR = SLICE_DIR / "visual_proof"
OUTPUT_DIR = SLICE_DIR / "generated_pdfs"
SUMMARY_PATH = SLICE_DIR / "jbalia_template_sample_renderer_hard_bind_summary.json"
...
def main() -> int:
    ...
    with app.test_client() as client:
        for slug, fighter_a, fighter_b, event_name, source_url in MATCHUPS:
            ...
            if response.status_code == 200 and data.get("ok") and output_path.exists():
                ...
                sections_ok = all(marker in text for marker in REQUIRED_SECTIONS)
    ...
    summary = {
        "slice": "button2-jbalia-template-sample-renderer-hard-bind-v1",
        ...
        "all_sections_present": all(r["all_sections_present"] for r in rows),
